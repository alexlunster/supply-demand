...
import { Event, events, InsertEvent, InsertUser, users } from "../drizzle/schema";
...
export async function insertEvents(eventList: InsertEvent[]): Promise<void> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot insert events: database not available");
    return;
  }
  ...
  try {
    ...
    console.error("[Database] Failed to insert events:", error);
    throw error;
  }
}
...
export async function deleteEventsByUser(userId: number): Promise<void> {
  const db = await getDb();
  if (!db) {
